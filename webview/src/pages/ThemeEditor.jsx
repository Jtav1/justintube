...
/**
 * Shared create/edit form for a theme, used both by the admin-only sitewide
 * theme manager (`/control-panel/themes/*`, `adminMode`) and any signed-in
 * user's personal theme manager (`/settings/themes/*`, `!adminMode`).
 *
 * In `adminMode`, a newly *created* theme is always sitewide (`system: true`
 * sent unconditionally, matching this form's original admin-only behavior).
 * When *editing* an existing theme, an admin additionally gets a "Publicly
 * usable" checkbox that can promote a private theme to sitewide, or reverse
 * a sitewide theme back to private — both directions are reversible. Since
 * `themeOwner` is overwritten in place on promotion (the original creator
 * isn't tracked separately), reversing a public theme makes it privately
 * owned by whichever admin unchecks the box, not its original creator.
 *
 * Outside `adminMode`, `system`/`isDefault` are never sent — the backend
 * always creates the theme owned by the caller — and only the caller's own
 * themes can be opened for editing (a private theme owned by someone else,
 * or a sitewide one, 404s here rather than opening a form that would only
 * get a 403 on submit).
 *
 * @param {{ adminMode?: boolean }} props
 */
function ThemeEditor({ adminMode = false }) {
  ...
  const backTo = adminMode ? '/control-panel' : '/settings/themes'
  ...
  useEffect(() => {
    function populateFromTheme() {
      ...
      if (target) {
        ...
        setMakePublic(target.themeOwner === PUBLIC_THEME_OWNER)
  ...
  if (adminMode ? !user || user.role !== 'admin' : !user) {
    return (
      <section className="settings-page">
  ...
  // Outside adminMode, a personal-theme editor may only open a theme this
  // user actually owns - a sitewide theme or someone else's private one
  // would only 403 on submit anyway, so treat it the same as "not found".
  const targetIsEditable =
    !targetTheme || adminMode || targetTheme.themeOwner === String(user.id)

  if (isEditMode && loaded && (!targetTheme || !targetIsEditable)) {
    return (
      <section className="settings-page">
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (adminMode) {
        if (isEditMode) {
          // Always send the checkbox's current state - both promoting to
          // public and reversing back to private are valid, reversible
          // server-side (see ThemeEditor's doc comment above).
          fields.system = makePublic
        } else {
          // Matches this form's original behavior: an admin creating a new
          // theme here always creates a sitewide one.
          fields.system = true
        }
    ...
    <section className="settings-page">
      <div className="settings-card">
        ...
        <form className="settings-form" onSubmit={handleSubmit}>
          ...
          <label htmlFor="theme-editor-description">Description</label>
          <textarea
          ...
          {adminMode && isEditMode && (
            <label className="admin-themes-checkbox">
              <input
                ...
                disabled={submitting}
              />
              Publicly usable (site-wide)
            </label>
          )}
          {adminMode && isEditMode && makePublic !== (targetTheme?.themeOwner === PUBLIC_THEME_OWNER) && (
            <p className="settings-status">
              {makePublic
                ? 'Will make this theme publicly usable for everyone on save.'
                : "Will make this theme private again on save, owned by your account. It won't be usable or visible to anyone else."}
            </p>
          )}
